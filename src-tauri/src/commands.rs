...
fn emit_error<R: Runtime>(app: &AppHandle<R>, message: &str) {
    let _ = app.emit("app_error", message.to_string());
}
...
#[tauri::command]
pub fn get_state(state: State<AppState>) -> Result<StateSnapshot, String> {
...
#[tauri::command]
pub fn reset_tournament<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
...
#[tauri::command]
pub fn register_player<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
    name: String,
    strategy: String,
...
#[tauri::command]
pub fn register_player_at_seat<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
    name: String,
    table_no: i64,
...
#[tauri::command]
pub fn eliminate_player<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
    player_id: i64,
) -> Result<(), String> {
...
#[tauri::command]
pub fn revive_player_at_seat<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
    player_id: i64,
    table_no: i64,
...
#[tauri::command]
pub fn balance_suggestions(state: State<AppState>) -> Result<Vec<MoveSuggestion>, String> {
    let conn = open_connection(&state.db_path)?;
    compute_balance_suggestions(&conn)
}

#[tauri::command]
pub fn close_table<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
    table_id: i64,
) -> Result<(), String> {
...
#[tauri::command]
pub fn clock_start<R: Runtime>(state: State<AppState>, app: AppHandle<R>) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
    let tournament = fetch_tournament(&conn)?.ok_or("Tournament not found")?;
...
#[tauri::command]
pub fn clock_pause<R: Runtime>(state: State<AppState>, app: AppHandle<R>) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
    let tournament = fetch_tournament(&conn)?.ok_or("Tournament not found")?;
...
#[tauri::command]
pub fn clock_next<R: Runtime>(state: State<AppState>, app: AppHandle<R>) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
    let tournament = fetch_tournament(&conn)?.ok_or("Tournament not found")?;
    let levels = fetch_levels(&conn, tournament.id)?;
...
#[tauri::command]
pub fn clock_prev<R: Runtime>(state: State<AppState>, app: AppHandle<R>) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
    let tournament = fetch_tournament(&conn)?.ok_or("Tournament not found")?;
...
#[tauri::command]
pub fn clock_trigger_break<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
...
#[tauri::command]
pub fn undo_last_event<R: Runtime>(
    state: State<AppState>,
    app: AppHandle<R>,
) -> Result<(), String> {
    let mut conn = open_connection(&state.db_path)?;
...
pub async fn save_export<R: Runtime>(
    ...
    let InvokeBody::Raw(content) = request.body() else {
        return Err("The export content must be sent as raw bytes".into());
    };
    ...
    let (filter_name, extension) = match extension.as_deref() {
        Some("csv") => ("CSV", "csv"),
        Some("pdf") => ("PDF", "pdf"),
        _ => return Err(format!("Unsupported export file type: {file_name}")),
    };
...
fn export_file_name(headers: &HeaderMap) -> Result<String, String> {
    let encoded = headers
...
pub(crate) fn start_clock_thread<R: Runtime>(app: AppHandle<R>, db_path: PathBuf) {
