...
- TypeScript bindings are generated by ts-rs into `src/bindings/` on `cargo test` (feature
  `ts`, always on for the crate's own tests). CI fails on binding drift.
...
with `icm::quote` through `Engine.quoteDeal`. The core returns codes and parameters, never text:
the UI turns every error and warning code into an English or French message (`src/i18n`).
